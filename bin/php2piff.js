...
const USAGE = fs.readFileSync(__dirname + '/php2piff-usage.txt', 'utf-8')
...
const compileFile = path =>
  fs
    ...
    .then(src => transpile(src, path))

const compileStdin = () => getStdin().then(transpile)
...
const updateFile = srcFilePath => {
  const outFilePath = srcFilePath.replace(/[.]php$/, '.piff')
...
const complainAboutSyntax = (srcFilePath, err) => {
  return fs.readFile(srcFilePath, 'utf-8').then(code => {
    let lineNumber = err.loc.start.line
    let columnNumber = err.loc.start.column
